...
class PerformEvaluation:
    ...
    def occupancy_rate(self):
        ...
        print("平均GPU内存占用率（MB）:", results['avg_gpu_memory_usage_MB'])
        print("平均GPU利用内存（MB）:", round(results['avg_gpu_utilization_percent'], 2))
        print("总时间（秒）:", round(results['total_time_seconds'], 2))

    def computation(self):

        if self.model is None:
            raise ValueError("Model not set. Use set_model() to set the model.")
